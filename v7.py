...
cat_vars_ohe_final['DEP_DELAY']=df['DEP_DELAY']
cat_vars_ohe_final['DEP_DEL15']=df['DEP_DEL15'].astype(int)
...
print(type(cat_vars_ohe_final['DEP_DELAY'][1]))
# In[] decisionTree
...
y_pred2 = np.where(y_pred < 0.2, 0, 1)

y_pred = np.array(y_pred)
y_test = np.array(y_test)

# Reshape y_pred and y_test into 1-dimensional arrays
y_pred_reshaped = y_pred.reshape((-1,))
y_test_reshaped = y_test.reshape((-1,))

# Create the DataFrame
result = pd.DataFrame({'Predicted': y_pred_reshaped, 'Actual': y_test_reshaped})

cm = confusion_matrix(y_test, y_pred)
printScore(y_test,y_pred2)
a=[]
...
#save and load
# final_model.save('model5')
final_model = tf.keras.models.load_model('model5')
